...
def update_hook(obj: Obj, destroy_list: List[Obj]) -> None:
    ...
    for obj2 in game.game.objects:
        ...
        if game_object.get_line_bb_intersection_point(obj.get_hook_attach_point(game.game), game.game.player_obj.get_pos_mid(), b_box):
    ...
    if obj.hook_attached_object is None:
        ...
        if obj.hook_attached_object is game.game.player_obj:
...
def draw_hook(obj: Obj) -> None:
    hook_point = obj.get_hook_attach_point(game.game)
    pyxel.line(hook_point[0], hook_point[1],
               game.game.player_obj.pos_x + HALF_GRID_CELL, game.game.player_obj.pos_y + HALF_GRID_CELL,
